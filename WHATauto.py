...
def getFile(downloadURL, cj):
	...
	req.add_header("User-Agent", "python-requests/2.1.0 CPython/2.7.5 Darwin/13.0.0")
	...
	def fdownload(self, vars):
		...
		if vars[1] and len(vars[1]) >= 2:
			if vars[1][0].lower() in self.fromalias.keys():
				...
				self.sendMsg('That site name does not seem valid. Type %sites to see a full list.   [pyWHATauto]'%site, target)
		else:
